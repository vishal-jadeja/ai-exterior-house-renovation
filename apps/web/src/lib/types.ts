...
export type Region = { id: string; label: RegionLabel; name: string; polygon: number[][]; pixel_area: number; bbox: number[]; confidence: number; source: string; version: number; is_active: boolean };
export type Material = { id: string; category: string; name: string; description: string; unit: string; coverage: number | null; coats: number; piece_area_sqft: number | null; pieces_per_box: number | null; wastage_pct: number; default_material_rate: number; default_labor_rate: number; currency: string; texture_key: string | null; texture_url?: string | null; color_hex: string | null; prompt_hint: string; applicable_labels: RegionLabel[]; durability_years: number | null; maintenance: string };
